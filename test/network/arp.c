...
u_char enet_src[6] = {0x11, 0x12, 0x13, 0x14, 0x15, 0x16};
u_char enet_dst[6] = {0x21, 0x22, 0x23, 0x24, 0x25, 0x26};
...
void mac(char *buf,u_char *uchar,int s)
{
	int k=0;
	u_char ubuf[strlen(buf)];
	strcpy(ubuf,buf);
...
void ethernet_packet_callback(u_char *argument,const struct pcap_pkthdr* pack,const u_char *content)
{
	int m=0,n;
...
int send_tcp_syn(){
	...
	printf("libnet 1.1 init: [Success]%d\n");
...
int pcap_tcp_syn()
{
	...
	while(1){
		...
		pcap_loop(handle,-1,ethernet_packet_callback,NULL);
	}	
}
